...
//    sendpkt_arg_t sendPktArg;
    ...
    //    printf("in overlay_sendpkt, nextNodeID is %d\n", sendPktArg->nextNodeID);
    sendPktArg->pkt = *pkt;
...
//        free(sendPktArg);
    ...
    segSize = sizeof(int) + sizeof(snp_hdr_t) + pkt->header.length;     //??????
//    printf("in overlay_sendpkt, pktSize is %d\n", segSize);
	if(send(overlay_conn, sendPktArg, segSize, 0) < 0) {
        free(sendPktArg);
		return -1;
	}
...
int forwardpktToSNP(snp_pkt_t* pkt, int network_conn)
{
    ...
    segSize = sizeof(snp_hdr_t) + pkt->header.length;
	if(send(network_conn, pkt, segSize, 0) < 0) {
		return -1;
	}
